...
export default function TabelaInstrutor({ instrutor, onEditar, onVisualizar }) {
  if (!instrutor.length) {
        ...
        >
          <div className="mb-2 sm:mb-0">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Eventos ministrados: <strong>{instrutor.eventosMinistrados?.length ?? 0}</strong>
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Média de avaliação: <strong>{instrutor.mediaAvaliacao?.toFixed(1) ?? "N/A"}</strong>
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400">
  Possui assinatura:{" "}
  ...
  <button
    ...
    aria-label={`Editar ${instrutor.nome}`}
  >
