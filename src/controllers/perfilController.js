...
// Helper: seta header que o frontend lê (X-Perfil-Incompleto)
  ...
  try {
    ...
    const { rows } = await db.query(
      ...
      SELECT id, nome, email, registro,
             cargo_id, unidade_id,
             -- 🔒 retorna somente a data em YYYY-MM-DD, sem fuso
...
async function atualizarMeuPerfil(req, res) {
  try {
    ...
    // Aceita campos parciais; string vazia → NULL
    const body = req.body || {};
    ...
    const data_nascimento       = norm(body.data_nascimento); // espera "YYYY-MM-DD"
    ...
    const pushDate = (col, val) => { sets.push(`${col} = $${vals.length + 1}::date`); vals.push(val); };
    ...
    if (data_nascimento !== undefined)      pushDate("data_nascimento", data_nascimento); // 👈 força tipo date
