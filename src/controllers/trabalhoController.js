...
function limitesDaChamada(ch) {
  return {
    ...
    consideracao: Number(ch?.limites?.consideracao) || 1000,
  };
}
...
async function atualizarNotasPersistidas50_50(subId) {
  ...
  try {
    ...
    const itensO = await db.any(
      ...
      FROM trabalhos_apresentacao_orais_itens
      WHERE submissao_id=$1
      ORDER BY avaliador_id
      `,
      [subId]
...
exports.criarSubmissao = async (req, res, next) => {
  try {
    ...
    if (status === "submetido") {
      ...
      assert(
        consideracao && withinLen(consideracao, lim.consideracao),
        `Considerações finais até ${lim.consideracao} caracteres.`
      );
    ...
    const ins = await db.one(
      `
      INSERT INTO trabalhos_submissao
      (usuario_id, chamada_id, titulo, inicio_experiencia, linha_tematica_id, linha_tematica_codigo,
       introducao, objetivos, metodo, resultados, consideracao, bibliografia, status)
      ...
      await t.none(
        ...
        UPDATE trabalhos_submissao
           SET titulo=$1, inicio_experiencia=$2, linha_tematica_id=$3, linha_tematica_codigo=$4,
               introducao=$5, objetivos=$6, metodo=$7, resultados=$8, consideracao=$9, bibliografia=$10,
...
/* ──────────────────────────────────────────────────────────────
   LISTAS / OBTENÇÃO
────────────────────────────────────────────────────────────── */
exports.minhassubmissao = async (req, res, next) => {
  try {
    const { userId } = getAuthUserId(req, res);
    if (userId === null) {
      const e = new Error("Não autorizado.");
...
// compat
exports.listarMinhas = exports.minhassubmissao;
...
exports.listarsubmissaoAdmin = async (req, res, next) => {
  ...
  try {
    ...
    console.log("[listarsubmissaoAdmin]", { chamadaId, total: rows.length, ms: Date.now() - t0 });
    res.json(rows);
  } catch (err) {
    console.error("[listarsubmissaoAdmin][erro]", { message: err.message, code: err.code, ms: Date.now() - t0 });
    next(err);
  }
};
...
exports.listarsubmissaoAdminTodas = async (_req, res, next) => {
  ...
  try {
    ...
    console.log("[listarsubmissaoAdminTodas]", { total: rows.length, ms: Date.now() - t0 });
    res.json(rows);
  } catch (err) {
    console.error("[listarsubmissaoAdminTodas][erro]", { message: err.message, code: err.code, ms: Date.now() - t0 });
    next(err);
  }
};
...
exports.avaliarEscrita = async (req, res, next) => {
  ...
  try {
    ...
    await db.none(
      `UPDATE trabalhos_submissao SET status='em_avaliacao', atualizado_em=NOW() WHERE id=$1 AND status='submetido'`,
      [subId]
    );
...
exports.avaliarOral = async (req, res, next) => {
  ...
  try {
    ...
    for (const it of itens) {
      ...
      await db.none(
        `
        INSERT INTO trabalhos_apresentacao_orais_itens
...
exports.consolidarClassificacao = async (req, res, next) => {
  ...
  try {
    ...
    const top40 = await db.any(
      `
      SELECT id FROM vw_submissao_consolidadas
      WHERE chamada_id=$1
    ...
    if (top40.length) {
      await db.none(
        `UPDATE trabalhos_submissao SET status='aprovado_exposicao' WHERE chamada_id=$1 AND id = ANY($2::int[])`,
    ...
    if (aprovadosOral.length) {
      await db.none(
        `UPDATE trabalhos_submissao SET status='aprovado_oral' WHERE chamada_id=$1 AND id = ANY($2::int[])`,
...
exports.definirStatusFinal = async (req, res, next) => {
  ...
  try {
    ...
    const observacao_admin = req.body?.observacao_admin || null;
    ...
    async function updateCompleto() {
      return db.none(
        ...
        UPDATE trabalhos_submissao
         ...
         WHERE id=$1
        `,
        [id, statusPrincipal, statusEscrita, statusOral, observacao_admin]
      );
    }
    ...
    async function updateSomenteStatus() {
      return db.none(
        ...
        UPDATE trabalhos_submissao
         ...
         WHERE id=$1
        `,
        [id, statusPrincipal, observacao_admin]
      );
    }
...
exports.contagemMinhasAvaliacao = async (req, res, next) => {
  try {
    ...
    console.error("Erro inesperado em contagemMinhasAvaliacao:", err);
    next(err);
  }
};
...
exports.listarsubmissaoDoAvaliador = async (req, res, next) => {
  ...
  try {
      ...
      SELECT
        ...
        CASE
          WHEN tsa.tipo = 'oral' THEN EXISTS (
            SELECT 1
            FROM trabalhos_apresentacao_orais_itens tai
            WHERE tai.submissao_id = s.id
    ...
    if (DBG) console.log("[listarsubmissaoDoAvaliador]", { uid, total: rows.length, ms: Date.now() - t0 });
    res.json(rows);
  } catch (err) {
    console.error("[listarsubmissaoDoAvaliador][erro]", err?.message || err);
    next(err);
  }
};
