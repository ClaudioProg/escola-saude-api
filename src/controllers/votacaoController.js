// ✅ controllers/votacaoController.js (versão premium)
...
exports.listarVotacaoAdmin = async (req, res, opts = {}) => {
  const { rows } = await query(`SELECT * FROM votacao ORDER BY criado_em DESC`, []);
...
exports.obterVotacaoAdmin = async (req, res, opts = {}) => {
  ...
  const op = await query(
    `SELECT * FROM votacao_opcao WHERE votacao_id=$1 ORDER BY ordem, id`,
    [id]
  );
...
exports.votar = async (req, res) => {
  ...
  const vRes = await query(`SELECT * FROM votacao WHERE id=$1`, [votacaoId]);
  ...
  try {
    ...
    const ins = await client.query(
      `INSERT INTO voto_submissao (votacao_id, usuario_id, ip, user_agent, cli_lat, cli_lng)
         VALUES ($1,$2,$3,$4,$5,$6)
    ...
    await client.query(
      `INSERT INTO voto_submissao_opcao (voto_id, opcao_id) VALUES ${values}`,
      [votoId, ...arr]
    );
