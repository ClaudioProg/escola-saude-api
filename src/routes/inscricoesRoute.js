// 📁 src/routes/inscricoesRoute.js — PREMIUM (robusto, consistente, sem surpresas)
...
if (typeof requireAuth !== "function") {
  console.error("[inscricoesRoute] authMiddleware inválido:", _auth);
...
if (typeof authorizeRoles !== "function") {
  console.error("[inscricoesRoute] authorizeRoles inválido:", _roles);
...
async function checarAcessoPorRegistroNaTurma(req, res, next) {
  ...
  try {
    ...
    if (!acesso?.ok) {
      ...
      if (IS_DEV) {
        console.warn("[inscricoesRoute] acesso negado", {
          rid,
          usuarioId,
          turmaId,
          eventoId: turma.evento_id,
    ...
    console.error("[inscricoesRoute] ERRO checarAcessoPorRegistroNaTurma:", e);
...
router.post(
  ...
  routeTag("inscricoesRoute:POST /"),
  checarAcessoPorRegistroNaTurma,
  handle(inscricoesController.inscreverEmTurma)
);
...
router.delete(
  "/minha/:turmaId",
  requireAuth,
  routeTag("inscricoesRoute:DELETE /minha/:turmaId"),
  handle(inscricoesController.cancelarMinhaInscricao)
);
...
router.delete(
  ...
  routeTag("inscricoesRoute:DELETE /:turmaId/usuario/:usuarioId"),
  handle(inscricoesController.cancelarInscricaoAdmin)
);
...
router.get(
  "/minhas",
  requireAuth,
  routeTag("inscricoesRoute:GET /minhas"),
  handle(inscricoesController.obterMinhasInscricoes)
);
...
router.get(
  ...
  routeTag("inscricoesRoute:GET /turma/:turma_id"),
  handle(inscricoesController.listarInscritosPorTurma)
);
...
router.get(
  "/conflito/:turmaId",
  requireAuth,
  routeTag("inscricoesRoute:GET /conflito/:turmaId"),
  checarAcessoPorRegistroNaTurma,
  handle(inscricoesController.conflitoPorTurma) // certifique-se de exportar no controller
);
...
router.delete(
  "/:id",
  requireAuth,
  routeTag("inscricoesRoute:DELETE /:id@legacy"),
  async (req, res) => {
    ...
    try {
      ...
      console.error("[inscricoesRoute] LEGADO DELETE /inscricoes/:id erro:", e);
